...
def fill_test_table(table) -> None:
    tests = [
        ...
        ("T16", "Boss血量", "进入第三关并查看顶部/头顶血条", "无", "最大生命3，两处血量一致", "通过（截图）"),
...
def build_report(args) -> Path:
    ...
    week1_intro = [
        ...
        "文字输出和动态刷新是第一周的另一个重点。settextstyle、settextcolor、textwidth和textheight共同决定字体、颜色与居中位置；使用TCHAR或宽字符串时需要让项目字符集和函数参数类型保持一致。对于会持续刷新的画面，应使用BeginBatchDraw和FlushBatchDraw，把一帧内容先绘制到缓冲区再整体显示，从而减少直接清屏造成的闪烁。动态时钟每帧读取系统时间，根据小时、分钟和秒数计算角度，再把角度转换为指针端点。这个过程把数学坐标、图形绘制、时间获取和对象职责结合起来，为后续游戏循环奠定了基础。",
    ]
    week1_difficult = [
        "第一周最容易出现的问题是字符集和图片路径。loadimage编译报错通常是项目字符集与字符串实参不匹配；即使编译通过，图片不显示也常由格式或路径造成。我的处理方法是整个解决方案统一使用Unicode字符集，涉及路径时使用std::wstring和宽字符串接口；资源文件统一采用PNG/JPG等EasyX支持的位图格式。最终项目没有写死C盘或D盘绝对路径，而是先取得EXE所在目录，再拼接assets下的英文子目录，同时由项目文件在编译后自动复制资源。整个输出目录复制到其他电脑后仍能按相对位置找到图片。",
    ...
    requirements = [
        ...
        "敌人与关卡需求：第一关以巡逻敌人为主，第二关加入根据曼哈顿距离追踪玩家的敌人，第三关加入3点生命Boss。普通敌人与Boss共享Enemy接口，Boss每次有效水浪只损失一点生命，血量在顶部状态条和对象头顶同步显示。",
    ...
    set_body(implementation_intro, "以下画面由当前Release x64程序的--capture-report模式自动绘制并保存，与正常运行使用同一套渲染函数和资源，不是概念占位图。")
...
// erase后unique_ptr自动析构真实派生敌人，不需要手工delete'''),
    ...
    set_body(test_intro, "测试覆盖构建配置、界面资源、输入兼容、地图边界、碰撞、连锁爆炸、强化上限、图片相对路径、角色差异和Boss多段受击。自动构建与报告截图能够直接证明的项目标记为通过，其余交互边界需要结合实际运行按表逐项复核。")
    ...
    summary = (
        "本项目完成了一个可独立运行的EasyX单人网格闯关游戏，包含四名差异化角色、多血量Boss、鼠标菜单、原创透明道具图标、行走精灵、背景音乐、音效、粒子效果、三关统计与自动报告截图。工程采用Visual Studio解决方案管理，Debug/Release和Win32/x64四种配置均能编译，资源通过项目目标自动复制，不依赖本机绝对路径。整体功能规模、界面完整度和可演示性达到最初设计目标。"
        ...
        "目前仍可继续改进：AI使用局部评分而不是完整寻路，在复杂木箱布局下可能选择次优路线；音乐使用简单波形生成，层次和循环衔接仍可提升；鼠标、极限碰撞和Boss连续命中等交互边界仍需结合实际运行逐项复核。后续可加入A*寻路和关卡编辑器。本次实践让我从基本图元与消息输入逐步过渡到完整游戏循环，也认识到画面、数据结构、对象职责、状态转换与资源路径需要保持一致。"
    )
    set_body(paragraph_by_text(document, "总结项目总体完成情况、技术难点、解决方法以及不足等，不少于500字。"), summary)

    # 模板尾部保留了一个空正文段落。总结恰好写满页面时，这个段落会在
    # Word 97-2003 兼容模式中单独占据下一页，因此保存前移除纯空尾段。
