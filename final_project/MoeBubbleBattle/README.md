...
《萌泡大作战》是使用 C++17、Visual Studio、EasyX 开发的原创 Q 版网格闯关游戏，支持单人挑战和同键盘本地双人合作。玩家放置水泡产生十字水浪，破坏木箱、收集强化道具并淘汰 AI 敌人，连续完成三关后获胜。
...
| 单人移动 | 方向键或 `W/A/S/D` |
| 单人放置水泡 | `Space` |
| 双人 P1 | `W/A/S/D` 移动，`Space` 放泡 |
| 双人 P2 | 方向键移动，`Enter` 放泡 |
| 暂停/继续 | `P` 或 `Esc` |
| 全屏/窗口切换 | `F11`（保持画面比例并自动适配鼠标坐标） |
| 结果界面重开 | `R` |

## 已实现功能

- 主菜单可选择单人闯关或本地双人合作；双人模式依次选择两名角色。
- 支持 `F11` 切换无边框全屏与窗口显示，使用等比缩放避免画面变形。
- 双人拥有独立生命、速度、水泡容量、爆炸范围、护盾和道具成长，任意一人存活即可继续。
- 三个关卡、逐步增加的 AI 敌人，以及第三关 3 点生命的“墨潮王”首领。
- 固定墙、可破坏木箱和安全出生区。
- 水泡容量、2.5 秒倒计时、十字水浪、连锁触发。
- 巡逻型与追踪型两类 AI 敌人。
- 范围、容量、速度、护盾四类随机道具，使用原创透明水彩图标而非字母占位符。
- 生命、分数、关卡、敌人数量、累计统计 HUD。
- 关卡完成、挑战失败、最终通关和重新开始。
- 原创菜单音乐、关卡音乐和七个操作/战斗音效。
- 四个可选择的原创 Q 版角色主题，具有全能、疾速、爆破、守护四套不同初始属性；角色原画已实际用于主菜单、角色选择、HUD、失败和通关界面。
- 地图角色使用四角色三帧透明行走精灵，以“回中—跨步—回中”的六阶段逻辑周期播放，并缩小跨步帧与校正重心，减轻左右摆动。
- 移动同时支持长按连续行走与轻点步进，并包含四方向优先、全通道转角辅助和脚底碰撞盒。
- 主菜单、角色卡、操作说明、暂停、过关/失败/通关和退出确认均支持鼠标悬停与点击。
...
- `Player`：可用不同编号和控制方案实例化两名玩家，分别维护生命、强化能力、水泡数量、三帧精灵动画和脚底碰撞盒。
- `Enemy`：AI 敌人抽象类。
- `PatrolEnemy`、`HunterEnemy`：通过重写决策函数实现运行时多态。
- `BossEnemy`：多血量首领，重写受击、得分、绘制与 AI 决策接口。
...
- `ItemIconAtlas`：加载四格透明道具图集并在地图、说明页和 HUD 中复用。
...
├─ TEST_PLAN.md    测试清单
├─ STORY_APPENDIX.md 世界观、角色与关卡剧情附录
└─ MoeBubbleBattle.sln
```
